...
class UBI(BaseComponent):
    ...
    Args:
        ...
        period (int): Length of a period in environment timesteps. UBI is
            distributed at the end of each period. Must be > 0. Default is 100.
    """
    ...
    def __init__(
        ...
        **base_component_kwargs
        ...
        # How many timesteps a "year" lasts (period in which UBI is paid once)
    ...
    def pay_ubi(self):
        ...
        world.planner.state["inventory"]["Coin"] -= self.basic_income_level * len(
            world.agents
        )
    ...
    def get_n_actions(self, agent_cls_name):
        ...
        The action space has as many actions as there are discretized UBI rates.
        """
        ...
        if agent_cls_name == "BasicPlanner":
            ...
            return [("UBI_payment_amount", self.n_disc_amts)]
    ...
    def generate_masks(self, completions=0):
        ...
        if self.ubi_cycle_pos != 1:
            ...
            masks[self.world.planner.idx] = {
                "UBI_payment_amount": (
                    self.disc_amts * len(self.world.agents) <= planner_coin
                ).astype(int)
            }
    ...
    def get_dense_log(self):
        ...
        Log UBI payments.
        Returns:
            ubi_payments (list): A list of UBI payments. Each entry corresponds to
                a single timestep. Entries are empty except for timesteps where a
                UBI period ended and UBI was distributed. For those timesteps, each
                entry contains the UBI payment amount.
